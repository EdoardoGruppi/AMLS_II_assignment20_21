...
def create_dataset(lr_path, batch_size, crop_size, rotation=True, flip=True):
    """
    ...
    data_set = Dataset.zip((load_dataset(lr_images_list), load_dataset(hr_images_list)))
...
def pre_processing(lr_image, hr_image, crop_size, rotation=True, flip=True, scale=4):
    """
    Processes a pair of low and high resolution images. It crops the same patch fromt the two images before rotating it
